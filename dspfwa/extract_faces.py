...
def main():
    ...
    if not collateonly:
        ...
        facedet.load_weights("/Users/lpdbrx/Desktop/EPFL/MA2c/DeepFakeDetection.nosync/icpr2020dfdc-master/blazeface/blazeface.pth")
        facedet.load_anchors("/Users/lpdbrx/Desktop/EPFL/MA2c/DeepFakeDetection.nosync/icpr2020dfdc-master/blazeface/anchors.npy")
        ...
        with ThreadPoolExecutor(threads) as p:
            for batch_idx0 in tqdm(np.arange(start=0, stop=len(df_videos_process), step=batch_size), desc='Extracting faces'):
            #for batch_idx0 in np.arange(start=0, stop=len(df_videos_process), step=batch_size):
    ...
    if index_enable:
        ...
        if offset is not None:
            if num is not None:
                facesdataset_path = facesdataset_path.parent.joinpath(str(facesdataset_path.parts[-1]).split('.')[0]+'/from_video_{}_to_video_{}.pkl'.format(offset, num+offset))
            else:
                facesdataset_path = facesdataset_path.parent.joinpath(str(facesdataset_path.parts[-1]).split('.')[0] + '/from_video_{}.pkl'.format(offset))
        elif num is not None:
            facesdataset_path = facesdataset_path.parent.joinpath(str(facesdataset_path.parts[-1]).split('.')[0] + '/from_video_{}_to_video_{}.pkl'.format(0, num))
